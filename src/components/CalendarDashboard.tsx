  ...
  return (
    <div className="h-screen flex flex-col bg-zinc-950 text-white selection:bg-orange-500/30 selection:text-white font-body overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-y-auto">
                    ...
                    <div className="max-h-[400px] overflow-y-auto p-4 space-y-3">
                        {filterStudio === 'all' ? (
                            ...
                            summaryData.bookedByStudio[filterStudio]?.slots?.map((b: any) => (
                                <div key={b.id} className="flex flex-col border-b border-zinc-800/30 pb-2 last:border-0 last:pb-0">
                                    ...
                                    <span className="text-[8px] font-black text-zinc-500 uppercase tracking-widest">{b.duration}</span>
                                </div>
                            ))
                        )}
                        ...
                        {filterStudio === 'all' ? (
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                {summaryData.availableByStudio[filterStudio]?.slots?.map((a: any, i: number) => (
                                    <div key={`${a.id}-${i}`} className="flex items-center justify-center bg-zinc-950/50 p-2 rounded-lg border border-zinc-800">
        ...
        <div className="flex-1 bg-zinc-950 p-6 pt-0">
            {loading ? (
              <div className="h-full flex flex-col items-center justify-center gap-6 bg-zinc-900/20 border border-zinc-900 rounded-[2rem]">
                        ...
                        <TableRow key={interval.start} className="border-none h-24">
                          ...
                          {filteredStudios.map((studio) => {
                            ...
                            if (slot.isBooked) {
                               ...
                               return (
                                 <TableCell key={`${interval.start}-${studio}`} rowSpan={slot.rowSpan || 1} className="p-1.5 align-top h-full border-r border-b border-zinc-900/30" style={{ height: '1px' }}>
                            ...
                            return (
                              <TableCell key={`${interval.start}-${studio}`} className="p-1.5 align-top h-full border-r border-b border-zinc-900/30">
                                <SlotCard slot={slot} existingBookings={studioBookings[studio] || []} />
                              </TableCell>
                            );
                          })}
              ...
              <div className="h-full flex flex-col items-center justify-center text-center gap-8 bg-zinc-900/10 border border-zinc-900 rounded-[3rem] backdrop-blur-sm">
